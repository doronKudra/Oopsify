...
export function FooterTrackPreview({ onAdd, onTilte, onArtist }) {
    ...
    // const [playingtrack, setPlayingtrack] = useSelector() //for now. not necessary
    const playingTrack = null
    if (!playingTrack) return
    return (
        <section className="footer-track-preview">
            <img src={playingTrack.imgUrl||defaultImg} alt="" width={'56px'} height={'56px'} />
            <div>
                <h5 onClick={onTilte}>{playingTrack.title}</h5>
                <h6 onClick={onArtist}>{playingTrack.artist}</h6>
            </div>
            <div>
